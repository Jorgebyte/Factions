...
final class UnclaimCommand extends BaseSubCommand
{
    ...
    public function __construct(
        ...
        parent::__construct("unclaim", "Unclaim the chunk you are standing on", ["unclaimchunk"]);
        $this->setPermission(Permissions::FACTIONS_COMMAND_UNCLAIM->value);
    }

    public function getPermission(): string
    {
        return Permissions::FACTIONS_COMMAND_UNCLAIM->value;
    }
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        Await::f2c(function () use ($sender) {
            /** @var Player $sender */
            $faction = $this->getFactionOrMessage($sender, $this->factionManager, LangKeys::UNCLAIM_NOT_IN_FACTION->value);
            if ($faction === null) {
                return;
            }

            if (!$this->checkRoleOrMessage($sender, $faction, [Role::LEADER, Role::COLEADER], LangKeys::UNCLAIM_NO_PERMISSION->value)) {
                return;
            }

            $result = yield from $this->territoryActionService->unclaimChunk($sender, $faction);
